...
Phase 6 baseline: `eval/baseline-phase6-scenarios.json`（20 題工作情境、auto 指標全 1.0、Gate C rubric 20/20）

本 repo 已達可運作的 MVP 階段，**Phase 6 已收尾**（golden regression + 真實工作情境驗收）。
...
- Scenario acceptance **20 題**已凍結於 `eval/baseline-phase6-scenarios.json`

Golden 驗證基準（`eval/baseline-phase5.json`）：

- `citation_hit_rate`: 1.0
- `refusal_accuracy`: 1.0
- `expected_refs_retrieved_rate`: 1.0

Scenario 驗證基準（`eval/baseline-phase6-scenarios.json`）：
...
| **rewrite_query** | `agent.FinRagAgent` | 以 LLM 將問題改寫為檢索用查詢（含 corpus 清單） |
...
| `FIN_RAG_MIN_RETRIEVAL_SCORE` | `0.028` | float | 最大融合 RRF 分數門檻；低於則重試或拒答 |
| `FIN_RAG_MAX_RETRIEVAL_ROUNDS` | `1` | int | 檢索信心不足時額外 rewrite+retrieve 次數 |
